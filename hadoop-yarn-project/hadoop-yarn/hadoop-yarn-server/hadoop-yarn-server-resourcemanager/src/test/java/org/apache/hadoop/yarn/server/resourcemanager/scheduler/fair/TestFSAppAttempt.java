...
public class TestFSAppAttempt extends FairSchedulerTestBase {
  ...
  public void testNoNextPendingAsk() {
    ...
    boolean unManagedAM = false;
    Mockito.when(appContext.getUnmanagedAM()).thenReturn(unManagedAM);
    LogAggregationContext logAggregationContext = Mockito.mock(LogAggregationContext.class);
    Mockito.when(appContext.getLogAggregationContext()).thenReturn(logAggregationContext);
    Mockito.when(rmApp.getApplicationSubmissionContext()).thenReturn(appContext);
    ...
    //Create FSSchedulerNode
    RMNode node = MockNodes.newNodeInfo(1,Resource.newInstance(8192, 8),1);
