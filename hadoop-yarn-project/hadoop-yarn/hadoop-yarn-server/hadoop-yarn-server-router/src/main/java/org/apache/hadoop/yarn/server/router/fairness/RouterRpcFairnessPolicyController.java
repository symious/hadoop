...
/**
 * Interface to define handlers assignment for specific subClusters.
 * This is needed to allow handlers provide a certain QoS for all subClusters
 ...
 * per subCluster and allow handlers to continue making downstream subClusters
 * calls if permissions are available, another implementation could use
 * preemption semantics and dynamically increase or decrease handlers
 * assigned per subCluster.
 */
...
public interface RouterRpcFairnessPolicyController {
   ...
   * connect to downstream subCluster. Controllers based on policies defined
   * and allocations done at start-up through assignHandlersToSubClusters,
   * may provide a permission or reject the request by throwing exception.
   *
   * @param subClusterId subClusterId for which a permission to continue is requested.
   * @return true or false based on whether permit is given.
   */
  boolean acquirePermit(String subClusterId);


  //Get all permits
  Map<String, Semaphore> getPermits();
   ...
   * particular subCluster. This would mean permissions getting available
   * for other handlers to request for this specific subCluster.
   *
   * @param subClusterId Name service id for which permission release request is made.
   */
  void releasePermit(String subClusterId);
