...
public class TestDistCpProcedure {
  ...
  public void testSuccessfulDistCpProcedure() throws Exception {
    ...
    FsPermission originalPerm = new FsPermission((short) 777);
  ...
  public void testInitDistCp() throws Exception {
    ...
    FedBalanceContext context = buildContext(src, dst, MOUNT);
    final DistCpProcedure dcProcedure =
        new DistCpProcedure("distcp-procedure", null, 1000, context);
    ...
    executeProcedure(dcProcedure, Stage.DIFF_DISTCP,
        new Call() {
          @Override
          public void execute() throws IOException, RetryException {
            dcProcedure.initDistCp();
          }
        });
    assertTrue(fs.exists(dst));
  ...
  public void testStageFinalDistCp() throws Exception {
    ...
    Path dst = new Path(testRoot, DSTDAT);
    // open files.
    final OutputStream out = fs.append(new Path(src, "a"));
    ...
    intercept(RemoteException.class, "LeaseExpiredException",
        "Expect RemoteException(LeaseExpiredException).",
        new Callable<Object>() {
          @Override
  ...
  public void testRecoveryByStage() throws Exception {
    ...
    executeProcedure(dcp[0], Stage.INIT_DISTCP, new Call() {
      @Override
      public void execute() throws IOException, RetryException {
        dcp[0].preCheck();
      }
    ...
    dcp[0] = serializeProcedure(dcp[0]);
    executeProcedure(dcp[0], Stage.DIFF_DISTCP, new Call() {
      @Override
      public void execute() throws IOException, RetryException {
        dcp[0].initDistCp();
      }
    ...
    dcp[0] = serializeProcedure(dcp[0]);
    executeProcedure(dcp[0], Stage.DISABLE_WRITE, new Call() {
      @Override
      public void execute() throws IOException, RetryException {
        dcp[0].diffDistCp();
      }
    ...
    dcp[0] = serializeProcedure(dcp[0]);
    executeProcedure(dcp[0], Stage.FINAL_DISTCP, new Call() {
      @Override
      public void execute() throws IOException, RetryException {
        dcp[0].disableWrite();
      }
    ...
    dcp[0] = serializeProcedure(dcp[0]);
    final OutputStream out = fs.append(new Path(src, "b/c"));
    executeProcedure(dcp[0], Stage.FINISH, new Call() {
      @Override
      public void execute() throws IOException, RetryException {
        dcp[0].finalDistCp();
      }
