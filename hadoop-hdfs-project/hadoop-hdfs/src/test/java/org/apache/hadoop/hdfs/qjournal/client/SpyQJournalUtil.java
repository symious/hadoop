...
public class SpyQJournalUtil {
  ...
  public static void mockJNWithEmptyOrSlowResponse(QuorumJournalManager manager, long startTxid) {
    ...
    Mockito.doAnswer(
        new Answer<ListenableFuture<GetJournaledEditsResponseProto>>() {
          @Override
          public ListenableFuture<GetJournaledEditsResponseProto> answer(
              InvocationOnMock invocation) {
            semaphore.release();
            return Futures.immediateFuture(GetJournaledEditsResponseProto.newBuilder()
                .setTxnCount(0).build());
          }
        }).when(spies.get(0))
    ...
    spyGetJournaledEdits(spies, 1, startTxid, new Runnable() {
      @Override
      public void run() {
        semaphore.release(1);
      }
    ...
    spyGetJournaledEdits(spies, 2, startTxid, new Runnable() {
      @Override
      public void run() {
  ...
  public static void spyGetJournaledEdits(List<AsyncLogger> spies,
      int jnSpyIdx, long fromTxId, final Runnable preHook) {
