...
public class TestSortLocatedBlockByLoad {
  ...
  private static DatanodeManager mockDatanodeManager(
    ...
    if (!multiDC) {
      ...
      conf.setClass(DFSConfigKeys.DFS_NET_TOPOLOGY_IMPL_KEY,
          DFSConfigKeys.DFS_NET_TOPOLOGY_WITH_DATA_CENTER_CLASS, NetworkTopology.class);
    }
    ...
    DatanodeManager dnm = Mockito.spy(new DatanodeManager(bm, fsn, conf));
  ...
  public void testOneDataCentreNoMultiDC() throws IOException {
    ...
    List<LocatedBlock> locatedBlocks = Arrays.asList(block);
  ...
  public void testTwoDataCentresNoMultiDCAndMultiDC() throws IOException {
    ...
    List<LocatedBlock> locatedBlocksNoMultiDC = Arrays.asList(block1);
    List<LocatedBlock> locatedBlocksMultiDC = Arrays.asList(block2);
