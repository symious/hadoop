...
public class TestCacheDirectives {
  ...
  private static void waitForCachedBlocks(NameNode nn,
    ...
    GenericTestUtils.waitFor(new Supplier<Boolean>() {
      ...
      public Boolean get() {
        ...
        namesystem.readLock(FSNamesystemLockMode.BM, "checkBlocks");
        try {
  ...
  private void checkPendingCachedEmpty(MiniDFSCluster cluster)
      throws Exception {
    Thread.sleep(1000);
    cluster.getNamesystem().readLock(FSNamesystemLockMode.BM, "checkPendingCachedEmpty");
    try {
  ...
  public void testExpiryTimeConsistency() throws Exception {
    ...
    GenericTestUtils.waitFor(() -> {
      ...
      ann.getNamesystem().readLock(FSNamesystemLockMode.FS, "expiryTimeConsistency");
      try {
        sbn.getNamesystem().readLock(FSNamesystemLockMode.FS, "expiryTimeConsistency");
        try {
