...
public class TestClientProtocolForPipelineRecovery {
  ...
  public void testPipelineRecoveryWithSlowNode() throws Exception {
    ...
    Configuration conf = new HdfsConfiguration();
    conf.setInt(HdfsClientConfigKeys.DFS_CLIENT_TREAT_SLOWNODE_AS_BADNODE_THRESHOLD_KEY, threshold);
  ...
  public void testAddingDatanodeDuringClosing() throws Exception {
    ...
    try {
      ...
      GenericTestUtils.waitFor(() -> ((DFSOutputStream) out.getWrappedStream()).getStreamer()
          .getNodes() != null, 100, 3000);
      ...
      new Thread(() -> {
        try {
          GenericTestUtils.waitFor(() -> ((DFSOutputStream) out.getWrappedStream()).getStreamer()
              .getStage() == BlockConstructionStage.PIPELINE_CLOSE, 100, 10000);
