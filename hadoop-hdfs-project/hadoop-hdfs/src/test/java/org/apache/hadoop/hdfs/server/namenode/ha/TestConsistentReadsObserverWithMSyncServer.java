...
/**
 * Test consistency of reads while accessing an ObserverNode with MSyncServer in NameNode.
 * The tests are based on traditional (non fast path) edits tailing.
 */
public class TestConsistentReadsObserverWithMSyncServer {
  ...
  public void testRequeueCall() throws Exception {
    ...
    configuration.set(prefix + CommonConfigurationKeys.IPC_SCHEDULER_IMPL_KEY,
        TestConsistentReadsObserver.TestRpcScheduler.class.getName());
  ...
  public void testCallFromNewClient() throws Exception {
    ...
    try {
      ...
      GenericTestUtils.waitFor(
          (Supplier<Boolean>) () -> readStatus.get() != 0, 100, 10000);
      assertEquals(1, readStatus.get());
    } finally {
  ...
  public void testMsyncFileContext() throws Exception {
    ...
    assertEquals("nn0 is not active", HAServiceProtocol.HAServiceState.ACTIVE, st.getState());
    st = nn2.getRpcServer().getServiceStatus();
    assertEquals("nn2 is not observer", HAServiceProtocol.HAServiceState.OBSERVER, st.getState());
