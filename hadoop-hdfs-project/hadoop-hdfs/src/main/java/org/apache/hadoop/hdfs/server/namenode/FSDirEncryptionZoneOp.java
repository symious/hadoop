...
final class FSDirEncryptionZoneOp {
  ...
  static EncryptionKeyInfo getEncryptionKeyInfo(FSNamesystem fsn,
    ...
    try {
      ...
      fsn.writeLock(FSNamesystemLockMode.FS, "getEncryptionKeyInfo");
  ...
  static String getKeyNameForZone(final FSDirectory dir,
    ...
    assert dir.getProvider() != null;
    final INodesInPath iip;
    dir.getFSNamesystem().readLock(FSNamesystemLockMode.FS, OperationName.GET_KEY_NAME_FOR_ZONE);
    try {
      ...
      dir.getFSNamesystem().readUnlock(FSNamesystemLockMode.FS, OperationName.GET_KEY_NAME_FOR_ZONE);
    }
  }
}
