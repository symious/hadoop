...
import static org.apache.hadoop.hdfs.client.HdfsClientConfigKeys.StripedRead.MOCK_FAILURE_DEFAULT;
...
public class DFSStripedInputStream extends DFSInputStream {
  ...
  /** Failure Indexes in one stripe which is mocked. **/
  private final boolean needMockMissing;
  ...
  DFSStripedInputStream(DFSClient dfsClient, String src,
    ...
    this.needMockMissing = dfsClient.getConfiguration()
        .getBoolean(MOCK_FAILURE, MOCK_FAILURE_DEFAULT);
  }
  ...
  private void readOneStripe(CorruptedBlocks corruptedBlocks)
    ...
    final long stripeLen = (long) cellSize * dataBlkNum;
  ...
  protected LocatedBlock refreshLocatedBlock(LocatedBlock block)
    ...
    LocatedStripedBlock lb = getBlockGroupAt(block.getStartOffset());
    // If indexing information is returned, iterate through the index array
    // to find the entry for position idx in the group
    int i = 0;
    for (; i < lb.getBlockIndices().length; i++) {
      if (lb.getBlockIndices()[i] == idx) {
        break;
      }
    }
  ...
  /**
   * @return true if we need mock one missing exception, else false.
   */
  public boolean needMockMissing() {
    return this.needMockMissing;
  }
}