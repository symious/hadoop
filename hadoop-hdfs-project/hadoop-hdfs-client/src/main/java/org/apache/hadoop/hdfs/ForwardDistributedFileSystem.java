...
public class ForwardDistributedFileSystem extends DistributedFileSystem {
  ...
  public RemoteIterator<PartialListing<FileStatus>> batchedListStatusIterator(
    ...
    if (useUnderFs()) {
      throw new UnderFSNotSupportedOperation(underUri,
          "batchedListStatusIterator(List<Path>)");
    } else {
  ...
  public RemoteIterator<PartialListing<LocatedFileStatus>> batchedListLocatedStatusIterator(
    ...
    if (useUnderFs()) {
      throw new UnderFSNotSupportedOperation(underUri,
          "batchedListLocatedStatusIterator(List<Path>)");
    } else {
  ...
  public ECTopologyVerifierResult getECTopologyResultForPolicies(
    ...
    if (useUnderFs()) {
      throw new UnderFSNotSupportedOperation(underUri,
          "getECTopologyResultForPolicies(String...)");
    } else {
  ...
  public RemoteIterator<OpenFileEntry> listOpenFiles(
      EnumSet<OpenFilesIterator.OpenFilesType> openFilesTypes) throws IOException {
    if (useUnderFs()) {
      throw new UnderFSNotSupportedOperation(underUri, "listOpenFiles(EnumSet<OpenFilesType>)");
    } else {
  ...
  public boolean hasPathCapability(final Path path, final String capability)
      throws IOException {
    if (useUnderFs()) {
      return underFs.hasPathCapability(path, capability);
    } else {
  ...
  public MultipartUploaderBuilder createMultipartUploader(final Path basePath)
    ...
    if (useUnderFs()) {
      throw new UnderFSNotSupportedOperation(underUri,
          "createMultipartUploader(Path)");
    } else {
      return super.createMultipartUploader(basePath);
    }
  }
}