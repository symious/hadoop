...
abstract class StripeReader {
  ...
  protected final int mockMissingIndex;
  ...
  StripeReader(AlignedStripe alignedStripe,
    ...
    boolean needMockMissing = this.dfsStripedInputStream.needMockMissing();
    if (needMockMissing) {
      this.mockMissingIndex = getMockIndex();
      DFSClient.LOG.debug("MockMissingIndex is " + this.mockMissingIndex);
    } else {
      this.mockMissingIndex = -1;
    }
  }

  private int getMockIndex() {
    int actualDataNum = 0;
    for (int index = 0; index < this.dataBlkNum; index++) {
      if (targetBlocks[index] != null) {
        actualDataNum++;
      }
    }
    int mockIndex = ThreadLocalRandom.current()
        .nextInt(this.dataBlkNum + this.parityBlkNum);
    if (mockIndex < this.dataBlkNum && mockIndex >= actualDataNum) {
      mockIndex = actualDataNum - 1;
    }
    return mockIndex;
  }
  ...
  boolean readChunk(final LocatedBlock block, int chunkIndex)
    ...
    // Only for test.
    if (chunkIndex == this.mockMissingIndex) {
      DFSClient.LOG.debug("MockMissingIndex is {} and " +
              "will make block {} missing.",
          this.mockMissingIndex, block);
      chunk.state = StripingChunk.MISSING;
      return false;
    }
  ...
  int[] prepareErasedIndices() {
    ...
    return Arrays.copyOf(decodeIndices, pos);
  }

  void clearFutures() {
    for (Future<BlockReadStats> future : futures.keySet()) {
      future.cancel(false);
    }
    futures.clear();
  }
