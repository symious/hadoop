...
final class FileChecksumHelper {
  ...
  static abstract class FileChecksumComputer {
    ...
    FileChecksumComputer(String src, long length,
        LocatedBlocks blockLocations, DFSClient client,
    ...
    void refetchBlocks() throws IOException {
      this.blockLocations = getClient().getBlockLocations(
          getSrc(), getLength());
    ...
    void compute() throws IOException {
      ...
      if (locatedBlocks == null || locatedBlocks.isEmpty()) {
        ...
        // buffer length value is changed in the future. This matters because the
      ...
      for (int i = 0; i < locatedBlocks.size() - 1; ++i) {
        ...
        LOG.debug(
            "Added blockCrc 0x{} for block index {} of size {}, " +
                "and current sumBlockLengths is {}",
            Integer.toString(blockCrc, 16), i,
            block.getBlockSize(), sumBlockLengths);
      }

      LocatedBlock nextBlock = locatedBlocks.get(locatedBlocks.size() - 1);
      long consumedLastBlockLength = Math.min(length - sumBlockLengths,
          nextBlock.getBlockSize());
      LocatedBlock lastBlockInFile = blockLocations.getLastLocatedBlock();
      if (nextBlock.equals(lastBlockInFile)) {
        ...
        long reportedLastBlockSize = lastBlockInFile.getBlockSize();
        consumedLastBlockLength = reportedLastBlockSize;
      ...
      switch (getBlockChecksumType()) {
        ...
        case COMPOSITE_CRC:
          ...
          if (LOG.isDebugEnabled()) {
            ...
            blockChecksumForDebug = Integer.toString(blockCrc, 16);
          }
  ...
  static class ReplicatedFileChecksumComputer extends FileChecksumComputer {
    ...
    ReplicatedFileChecksumComputer(String src, long length,
        LocatedBlocks blockLocations, DFSClient client,
        ChecksumCombineMode combineMode) throws IOException {
      super(src, length, blockLocations, client, combineMode);
    }
    ...
    private boolean checksumBlock(LocatedBlock locatedBlock) {
      ...
      long requestLength = Math.min(getRemaining(), block.getNumBytes());
      setRemaining(getRemaining() - requestLength);
      DatanodeInfo[] datanodes = locatedBlock.getLocations();
      //try each datanode location of the block
      boolean done = false;
      for (int j = 0; !done && j < datanodes.length; j++) {
        try {
          tryDatanode(locatedBlock, datanodes[j], requestLength);
          done = true;
    ...
    private void tryDatanode(LocatedBlock locatedBlock,
        DatanodeInfo datanode, long requestLength) throws IOException {
      ExtendedBlock block = locatedBlock.getBlock();
      BlockChecksumHelper blockChecksumHelper = new BlockChecksumHelper(
          getSrc(), getClient(), locatedBlock, datanode);
      BlockOpResponseProto reply = blockChecksumHelper.compute(
          getBlockChecksumType(), requestLength);
      ...
      LOG.debug("got reply from {}: blockChecksum=0x{}, blockChecksumType={}, requestLength={}",
          datanode, blockChecksumForDebug, getBlockChecksumType(), requestLength);
    }
  }
  ...
  static class StripedFileNonStripedChecksumComputer
    ...
    StripedFileNonStripedChecksumComputer(
        String src, long length, LocatedBlocks blockLocations,
        DFSClient client, ErasureCodingPolicy ecPolicy,
        ChecksumCombineMode combineMode) throws IOException {
      super(src, length, blockLocations, client, combineMode);
    ...
    @Override
    void checksumBlocks() throws IOException {
      int tmpTimeout = getClient().getConf().getChecksumEcSocketTimeout() +
    ...
    private boolean checksumBlockGroup(LocatedStripedBlock blockGroup) {
    ...
    private void tryDatanode(LocatedStripedBlock blockGroup,
        StripedBlockInfo stripedBlockInfo, DatanodeInfo datanode,
