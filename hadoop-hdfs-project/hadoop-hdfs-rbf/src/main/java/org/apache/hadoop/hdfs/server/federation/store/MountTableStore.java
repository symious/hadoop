...
import org.apache.hadoop.hdfs.server.federation.router.MountTableRefreshService;
...
public abstract class MountTableStore extends CachedRecordStore<MountTable>
  ...
  private MountTableRefreshService refreshService;

  public MountTableStore(StateStoreDriver driver) {
    super(MountTable.class, driver);
  }

  public void setRefreshService(MountTableRefreshService refreshService) {
    this.refreshService = refreshService;
  }

  /**
   * Update mount table cache of this router as well as all the routers.
   */
  protected void updateCache() {
    if (refreshService != null) {
      try {
        ...
        LOG.error(
            "Can not refresh munt table cache as state store is not available",
            e);
      }
    }
  }
