...
public class TestObserverWithSyncFlagInRouter {
  ...
  public void testSyncForwardByClient() throws Exception {
    ...
    List<? extends FederationNamenodeContext> namenodes = routerContext
        ...
        .getNamenodesForNameserviceId(
            cluster.getNameservices().get(0), true);
    ...
    // Send read request to observer but no msync
    fileSystem.open(path).close();
    // Create, close, msync call should send to active
    assertEquals("Three calls should send to active", rpcCountForActive,
        rpcMetrics.getProxyOpActiveCommunicate());
    // getBlockLocations should send to observer
    assertEquals("One call should send to observer",
        rpcCountForObserver + 1, rpcMetrics.getProxyOpObserverCommunicate());
    assertTrue(Time.monotonicNow() - beginTime < msyncPeriod);

    CallerContext msyncCallerContext = new CallerContext
        .Builder("test_needSyncObserverRead").build();
    CallerContext.setCurrent(msyncCallerContext);

    // Send read request to observer and need msync
    fileSystem.open(path).close();
    // Create, close, msync * 2 call should send to active
    assertEquals("Three calls should send to active", rpcCountForActive + 1,
        rpcMetrics.getProxyOpActiveCommunicate());
    // getBlockLocations should send to observer
    assertEquals("One call should send to observer",
        rpcCountForObserver + 2, rpcMetrics.getProxyOpObserverCommunicate());
    assertTrue(Time.monotonicNow() - beginTime < msyncPeriod);


    CallerContext normalCallerContext = new CallerContext
        .Builder("test").build();
    CallerContext.setCurrent(normalCallerContext);

    // Send read request to observer and don't need msync
    fileSystem.open(path).close();
    // Create, close, msync * 2 call should send to active
    assertEquals("Three calls should send to active", rpcCountForActive + 1,
        rpcMetrics.getProxyOpActiveCommunicate());
    // getBlockLocations should send to observer
    assertEquals("One call should send to observer",
        rpcCountForObserver + 3, rpcMetrics.getProxyOpObserverCommunicate());
    assertTrue(Time.monotonicNow() - beginTime < msyncPeriod);


    CallerContext abnormalCallerContext = new CallerContext
        .Builder(null).build();
    CallerContext.setCurrent(abnormalCallerContext);
    ...
    // getBlockLocations should send to observer
    assertEquals("One call should send to observer",
        rpcCountForObserver + 4, rpcMetrics.getProxyOpObserverCommunicate());
    assertTrue(Time.monotonicNow() - beginTime < msyncPeriod);
