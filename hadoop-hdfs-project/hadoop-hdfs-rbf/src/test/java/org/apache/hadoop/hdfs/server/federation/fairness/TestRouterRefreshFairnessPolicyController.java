...
public class TestRouterRefreshFairnessPolicyController {
  ...
  public void testRefreshStaticDynamic() throws Exception {
    ...
    routerContext.getConf().setLong(
        RBFConfigKeys.DFS_ROUTER_DYNAMIC_FAIRNESS_CONTROLLER_REFRESH_INTERVAL_SECONDS_KEY, 3);
    client.refreshFairnessPolicyController(routerContext.getConf());
    ...
    assertEquals("{\"concurrent\":1,\"ns0\":6,\"ns1\":3}",
        controller.getAvailableHandlerOnPerNs());
    ...
    routerContext.getConf().set(RBFConfigKeys.DFS_ROUTER_FAIRNESS_POLICY_CONTROLLER_CLASS,
        DynamicRouterRpcFairnessPolicyController.class.getCanonicalName());
    routerContext.getConf().setLong(
        RBFConfigKeys.DFS_ROUTER_DYNAMIC_FAIRNESS_CONTROLLER_REFRESH_INTERVAL_SECONDS_KEY, 2);
    client.refreshFairnessPolicyController(routerContext.getConf());
  ...
  private List<Thread> makeDummyInvocations(final RouterRpcClient client,
      final int nThreads, final String namespace) throws IOException {
    final RemoteMethod method = new RemoteMethod("renewLease",
        new Class<?>[] {String.class, List.class}, "helloWorld", null);
    List<Thread> threadAcquirePermits = new ArrayList<>();
    for (int i = 0; i < nThreads; i++) {
      Thread threadAcquirePermit = new Thread(() -> {
        try {
          client.invokeSingle(namespace, method);
        } catch (IOException e) {
          e.printStackTrace();
        }
      });
