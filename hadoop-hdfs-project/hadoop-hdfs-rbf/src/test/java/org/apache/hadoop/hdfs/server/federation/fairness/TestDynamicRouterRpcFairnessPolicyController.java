...
public class TestDynamicRouterRpcFairnessPolicyController {
  ...
  private void verifyDynamicControllerSimple(boolean manualRefresh)
    ...
    Map<String, AtomicLong> rejectedPermitsPerNs = new HashMap<>();
    Map<String, AtomicLong> acceptedPermitsPerNs = new HashMap<>();
  ...
  private void verifyRemainingPermitCounts(int[] remainingPermitCounts,
                                           String[] nss, DynamicRouterRpcFairnessPolicyController controller, boolean lockController) {
  ...
  private void injectDummyMetrics(Map<String, AtomicLong> metrics, String ns,
                                  long value) {
    if (metrics.containsKey(ns)) {
      metrics.get(ns).addAndGet(value);
    } else {
      metrics.put(ns, new AtomicLong(value));
    }
  }
