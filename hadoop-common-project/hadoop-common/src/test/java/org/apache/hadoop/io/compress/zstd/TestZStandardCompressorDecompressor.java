...
public class TestZStandardCompressorDecompressor {
  ...
  public void testCompressorDecompressorWithFinish() throws Exception {
    ...
    try {
      DataOutputBuffer compressedDataBuffer = new DataOutputBuffer();
      CompressionOutputStream deflateFilter =
          new CompressorStream(compressedDataBuffer, new ZStandardCompressor(), bufferSize);
      ...
      CompressionInputStream inflateFilter = new DecompressorStream(deCompressedDataBuffer,
          new ZStandardDecompressor(bufferSize), bufferSize);
      ...
      byte[] result = new byte[byteSize];
      inflateIn.read(result);
      assertArrayEquals("original array not equals compress/decompressed array", bytes, 
          result);
    } finally {
      IOUtils.closeStream(deflateOut);
      IOUtils.closeStream(inflateIn);
  ...
  public void testReadingWithAStream() throws Exception {
    ...
    try (CompressionInputStream cis = codec.createInputStream(inputStream, decompressor);
         ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
      byte[] buffer = new byte[100];
      int n;
