...
class Display extends FsCommand {
  ...
  public static class Checksum extends Display {
    ...
    public static final String USAGE = "[-v] [-l <input length>] <src> ...";
    public static final String DESCRIPTION =
    ...
    protected void processOptions(LinkedList<String> args)
      ...
      cf.addOptionWithValue("l");
      cf.parse(args);
      displayBlockSize = cf.getOpt("v");
      String fileLengthOp = cf.getOptValue("l");
      if (fileLengthOp != null && !fileLengthOp.isEmpty()) {
        long fileLength = Long.parseLong(fileLengthOp);
        if (fileLength > 0) {
    ...
    protected void processPath(PathData item) throws IOException {
      ...
      FileChecksum checksum = item.fs.getFileChecksum(item.path, this.fileLength);
