...
public final class HHUtil {
  ...
  public static int[] initPiggyBackFullIndexVec(
      int numDataUnits, int[] piggyBackIndex) {
  ...
  public static ByteBuffer[] getPiggyBacksFromInput(
      ByteBuffer[] inputs, int[] piggyBackIndex,
      int numParityUnits, int pgIndex,
      RawErasureEncoder encoder) throws IOException {
    ...
    for (int i = 0; i < numParityUnits - 1; ++i) {
      ...
      for (ByteBuffer byteBuffer : tempOutput) {
        byteBuffer.clear();
      }
    }
  ...
  public static ByteBuffer allocateByteBuffer(
      boolean useDirectBuffer, int bufSize) {
    if (useDirectBuffer) {
  ...
  public static ByteBuffer getPiggyBackForDecode(
      ByteBuffer[][] inputs, ByteBuffer[][] outputs,
      int pbParityIndex, int numDataUnits,
      int numParityUnits, int pbIndex) {
    ByteBuffer firstValidInput = HHUtil.findFirstValidInput(inputs[0]);
    int bufSize = firstValidInput.remaining();

    ByteBuffer piggybacks = allocateByteBuffer(
        firstValidInput.isDirect(), bufSize);
