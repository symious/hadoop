...
public class HHXORErasureDecodingStep extends HHErasureCodingStep {
  ...
  private final int[] piggyBackIndex;
  private final int[] piggyBackFullIndex;
  private final int[] erasedIndexes;
  private final RawErasureDecoder rsRawDecoder;
  private final RawErasureEncoder xorRawEncoder;
  ...
  private void performCoding(ByteBuffer[] inputs, ByteBuffer[] outputs)
    ...
    if (erasedIndexes.length == 1 && erasedIndexes[0] < numDataUnits) {
      ...
      doDecodeSingle(newIn, newOut, erasedIndexes[0],
          bufSize, fisrtValidInput.isDirect());
    } else {
  ...
  private void doDecodeSingle(ByteBuffer[][] inputs, ByteBuffer[][] outputs,
      int erasedLocationToFix, int bufSize, boolean isDirect)
      throws IOException {
    final int numDataUnits = rsRawDecoder.getNumDataUnits();
    final int numParityUnits = rsRawDecoder.getNumParityUnits();
    final int subPacketSize = getSubPacketSize();
    ...
    if (isDirect) {
      ...
      int idxToWrite = 0;
      doDecodeByPiggyBack(inputs[0], tmpOutputs[0][idxToWrite],
          piggyBack, erasedLocationToFix);
    } else {
      ByteBuffer buffer;
  ...
  private void doDecodeByPiggyBack(
      ByteBuffer[] inputs, ByteBuffer outputs,
      ByteBuffer piggyBack, int erasedLocationToFix) {
  ...
  private void doDecodeByPiggyBack(byte[][] inputs, int[] inputOffsets,
      byte[] outputs, int outOffset, byte[] piggyBack,
  ...
  private void doDecodeMultiAndParity(
      ByteBuffer[][] inputs, ByteBuffer[][] outputs,
      int[] erasedLocationToFix, int bufSize)
      throws IOException {
    final int numDataUnits = rsRawDecoder.getNumDataUnits();
